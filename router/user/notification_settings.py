...
def get():
    try:
        ...
        result = db.user_settings.get_multiple_settings(user_uuid, ['notification_login', 'notification_door_access'])
...
def update():
    try:
        ...
        result = db.user_settings.set_setting(user_uuid, setting_key, is_enabled, 'boolean')
        if not result.success:
            return utils.ResultDTO(code=400, message=result.detail, success=False).to_response()
