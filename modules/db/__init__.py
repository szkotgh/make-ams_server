...
def get_db_connection():
    conn = sqlite3.connect('./modules/db/main.db')
...
CREATE TABLE IF NOT EXISTS users (
    ...
    salt VARCHAR(32) NOT NULL,                              -- 유저 비밀번호 솔트값
    user_name VARCHAR(10) NOT NULL,                         -- 유저 이름(정확인 실제 이름)
    is_activated BOOLEAN NOT NULL DEFAULT FALSE,            -- 유저 활성화 여부(0: 비활성화, 1: 활성화. 회원가입 후 관리자가 승인해야 계정 로그인 가능)
    is_admin BOOLEAN NOT NULL DEFAULT FALSE,                -- 관리자 여부
    is_teacher BOOLEAN NOT NULL DEFAULT FALSE,              -- 선생님 여부
...
CREATE TABLE IF NOT EXISTS auth_button (
    is_activated BOOLEAN NOT NULL DEFAULT FALSE,            -- 버튼 인증 활성화 여부
    update_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP  -- 마지막 버튼 인증 시각
);
...
CREATE TABLE IF NOT EXISTS auth_qr (
    ...
    is_used BOOLEAN NOT NULL DEFAULT FALSE,                 -- QR 인증 사용 여부
...
-- 게스트 인증 QR 코드 인증을 위한 테이블(게스트를 위한 QR 생성 후 만료 시간까지 계속 사용)
...
CREATE TABLE IF NOT EXISTS auth_nfc (
    ...
    is_activated BOOLEAN NOT NULL DEFAULT FALSE,            -- NFC 활성화 여부
    is_lost BOOLEAN NOT NULL DEFAULT FALSE,                 -- NFC 분실 여부(1일 경우 활성화 되어 있어도 인증 불가)
...
CREATE TABLE IF NOT EXISTS device (
    device_id INTEGER PRIMARY KEY AUTOINCREMENT,              -- 장치 ID(자동 증가)
    auth_token VARCHAR(32) NOT NULL,                          -- 장치 인증 토큰(장치가 서버에 요청할 때 Bearer 토큰으로 사용)
    device_name VARCHAR(32) NOT NULL,                         -- 장치 이름(예: 1층 출입문)
    device_status VARCHAR(16) NOT NULL DEFAULT 'close',       -- 장치 상태(예: open, restrict, close)
    device_enabled BOOLEAN NOT NULL DEFAULT TRUE,             -- 장치 활성화 여부(장치가 꺼져있을 때는 인증 불가)
    
    button_enabled BOOLEAN NOT NULL DEFAULT FALSE,            -- 장치 버튼 인증 활성화 여부
    nfc_enabled BOOLEAN NOT NULL DEFAULT FALSE,               -- 장치 NFC 인증 활성화 여부
    qr_enabled BOOLEAN NOT NULL DEFAULT FALSE,                -- 장치 QR 인증 활성화 여부
    
    update_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,   -- 마지막 정보 업데이트 시각
    create_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP    -- 장치 정보 생성 시각
    -- door_remote_open BOOLEAN NOT NULL DEFAULT FALSE,          이거는 서버 메모리에서 관리하는 것으로 변경. status를 장치가 한번 get하게 되면 door_remote_open는 다시 0으로 바뀜. 관리자만 사용 가능한 기능이며 1로 설정해서 장치가 get할 때 1로 바뀌면 문이 열림.
);
    ''')
